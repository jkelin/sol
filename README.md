...
`bun run build:packages` compiles `@soljs/sol`, `@soljs/compiler`, and `@soljs/solkit` into each package's `dist/` directory. Every package contains one tree-shakeable ESM bundle (`index.js`) and one rolled-up declaration file (`index.d.ts`); all existing package subpaths resolve to those files. The build formats both generated files with Oxfmt and fails if formatting does not complete. Third-party dependencies remain external, and each generated manifest declares `"sideEffects": false` so consumer bundlers can remove unused exports. The root `prepublishOnly` lifecycle runs that complete build before `bun run publish:packages` uses npm's CLI to publish the three generated packages in dependency order. Each generated package also rebuilds its source package through its own `prepublishOnly` lifecycle before packing, so publishing cannot start with missing, stale, or unformatted output.
