...
`bun run build` writes one tree-shakeable ESM bundle (`dist/index.js`) and one rolled-up declaration file (`dist/index.d.ts`), then formats both files with Oxfmt. Every published runtime subpath resolves to those two files.
...
- `router.ts` validates and installs compiler-generated route manifests, loads matched route-file chunks before schema resolution, connects definitions to
...
The compiler's development entry calls `installDevtools()` from `@soljs/sol/devtools` to install `globalThis.__sol` and a Shadow DOM panel. Calling the exported function directly provides the same explicit installation path. Compiler-emitted
