...
const virtualRoutes = "virtual:frontend-framework/routes";
...
function routeManifest(files: readonly string[]): string {
  const imports = files.map((file, index) => {
    ...
    return `import * as __ff_route_module_${index} from ${JSON.stringify(specifier)};`;
  });
  const modules = files.map((_, index) => `__ff_route_module_${index}`).join(", ");
  return `${imports.join("\n")}
import { isRouteDefinition as __ff_is_route } from "frontend-framework/runtime";
const __ff_modules = [${modules}];
export default __ff_modules.flatMap(module => Object.values(module).filter(__ff_is_route));`;
}
